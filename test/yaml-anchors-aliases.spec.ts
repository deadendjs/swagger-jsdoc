...
import referenceSpecification from '../examples/yaml-anchors-aliases/reference-specification.json';
...
describe('Example for using anchors and aliases in YAML documents', () => {
  ...
  it('should not throw errors that have been cleaned up', async () => {
    const config = {
      swaggerDefinition: {
        info: {
          title: 'Example with anchors and aliases',
    ...
    await expect(swaggerJsdoc(config)).resolves.not.toThrow();
  });
});
