...
describe('Utilities module', () => {
  ...
  describe('extractAnnotations', () => {
    it('should extract jsdoc comments by default', () => {
      expect(utils.extractAnnotations(require.resolve('../examples/app/routes2.js'))).toEqual({
        yaml: [],
        jsdoc: [
    ...
    it('should extract data from YAML files', () => {
      expect(utils.extractAnnotations(require.resolve('../examples/app/parameters.yaml'))).toEqual({
        yaml: [
      ...
      expect(utils.extractAnnotations(require.resolve('../examples/app/parameters.yml'))).toEqual({
        yaml: [
          'parameters:\n  username:\n    name: username\n    description: Username to use for login.\n    in: formData\n    required: true\n    type: string\n'
        ],
        jsdoc: []
    ...
    it('should extract jsdoc comments from coffeescript files/syntax', () => {
      expect(utils.extractAnnotations(require.resolve('../examples/app/route.coffee'))).toEqual({
        yaml: [],
        jsdoc: [
    ...
    it('should return empty arrays from empty coffeescript files/syntax', () => {
      expect(utils.extractAnnotations(require.resolve('./fixtures/empty-file.coffee'))).toEqual({
        yaml: [],
        jsdoc: []
      });
    });
    ...
    it('should extract jsdoc comments from empty javascript files/syntax', () => {
      expect(utils.extractAnnotations(require.resolve('./fixtures/empty-file.js'))).toEqual({
        yaml: [],
        jsdoc: []
      });
    });
